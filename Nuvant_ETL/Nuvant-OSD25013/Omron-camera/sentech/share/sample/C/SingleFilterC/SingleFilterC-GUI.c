...
void* acquisitionWorker(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (isImagePresent)
        {
            ...
            eStApiCError = psApiFunctions->StApi->IStFilter->Filter(&sIStFilterHandle, &sIStImageHandle);
            if (eStApiCError != StApiCError_NoError) break;
...
int main(int argc, char **argv)
{
	...
	for (;;)
	{
		...
		eStApiCError = psApiFunctions->StApi->IStFilter->CreateIStFilter(StFilterType_EdgeEnhancement, &sIStFilterHandle);
		if (eStApiCError != StApiCError_NoError) break;

		/* Configure the EdgeEnhancement filter. */
		eStApiCError = psApiFunctions->StApi->IStEdgeEnhancementFilter->SetStrength(&sIStFilterHandle, 5);
		if (eStApiCError != StApiCError_NoError) break;
		...
		eStApiCError = psApiFunctions->StApi->IStFilter->GetIStFilterInfo(&sIStFilterHandle, &sIStFilterInfoHandle);
		if (eStApiCError != StApiCError_NoError)	break;
		...
		eStApiCError = psApiFunctions->StApi->IStFilter->GetINodeMap(&sIStFilterHandle, &sINodeMapHandle);
		if (eStApiCError != StApiCError_NoError)	break;
	...
	if (sIStFilterHandle.Handle)
	{
		psApiFunctions->StApi->IStFilter->Release(&sIStFilterHandle);
	}
