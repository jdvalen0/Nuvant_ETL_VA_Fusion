...
void acquisitionWorker(CIStDevicePtr *pIStDevice, CIStDataStreamPtr *pIStDataStream, CIStImageDisplayWndPtr *pIStImageDisplayWnd, bool *isCompleted)
{
    ...
    while ((*pIStDataStream)->IsGrabbing())
    {
        ...
        try
        {
            pIStStreamBuffer.Reset((*pIStDataStream)->RetrieveBuffer(5000));
        }
        catch(GenICam::GenericException &x)
        {
            cout << x.GetDescription() << std::endl;
        }
...
int main(int argc, char **argv)
{
    try
    {
        ...
        CIStDevicePtr pIStDevice(pIStSystem->CreateFirstIStDevice(isMonitor ? GenTL::DEVICE_ACCESS_READONLY : GenTL::DEVICE_ACCESS_CONTROL));
