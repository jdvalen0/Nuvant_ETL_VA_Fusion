/*!
\file FrameStartTrigger-GUI.cpp
\brief 
    ...
    if(pIStCallbackParamBase->GetCallbackType() == StCallbackType_GenTLEvent_DataStreamNewBuffer)
    {
        ...
        try
        {
            ...
            if (pIStStreamBuffer->GetIStStreamBufferInfo()->IsImagePresent())
            {
                ...
                // Note: in the official document of Qt, GUI shall be controlled through main thread.
                // Setting the GUI through different thread might not work properly.
...
int main(int, char **)
{
    try
    {
        ...
        // Register a callback function. When an event occurs for Data strem, function registered is called.
#ifdef ENABLED_CLASS_METHOD_TYPE_CALLBACK
        ...
        // Process event GUI until all Window object is closed by user
        processEventGUI(true);
