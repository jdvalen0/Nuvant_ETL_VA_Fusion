...
const uint64_t nCountOfImagesToGrab = GENTL_INFINITE;// 5000;
...
void MakeGammaTable(double dblGammaCorrectionValue, vector<uint16_t> &vecGammaTable)
{
    if (0 < dblGammaCorrectionValue)
    {
        ...
        for (size_t i = 0; i < vecGammaTable.size(); ++i)
        {
            ...
            vecGammaTable[i] = ((dblOutput * nMaxValue) + 0.5);
        }
    }
    else
    {
...
{
    uint8_t *pbyteBuffer = (uint8_t*)pwBuffer;
...
void SendGammaTable(GenApi::CNodeMapPtr pINodeMap, vector<uint16_t> &vecGammaTable, bool enabledSwapEndian)
{
    ...
    const size_t nMaxValue = pIInteger_LUTIndex->GetMax();
    ...
    {
        if ((size_t)pIRegister_LUTValueAll->GetLength() == (nMaxValue + 1) * sizeof(uint16_t))
        {
            bUseValueAll = true;
        }
    }
...

{
    ...
    else
    {
        ...
        {
            pIInteger_LUTIndex->SetValue(i);
            vecGammaTable[i] = pIInteger_LUTValue->GetValue();
        }
    }
}
...
void UpdateLUTGraph(CBooleanPtr pIBoolean_LUTEnable, CIntegerPtr pIInteger_LUTIndex, CRegisterPtr pIRegister_LUTValueAll, CIntegerPtr pIInteger_LUTValue, IStGraphDisplayWnd *pIStGraphDisplayWnd, bool enabledSwapEndian)
{
    ...
    uint32_t * const pdwBuffer = (uint32_t*)pIStGraphData->GetDataPtr();
    for (size_t i = 0; i <= nMaxValue; ++i)
    {
...
void processGamma(CIStDevicePtr *pIStDevice, CIStGraphDisplayWndPtr *pIStGraphDisplayWnd, bool enabledSwapEndian, bool *isCompleted)
{
    const double pdblGammaCorrectonValues[11] = { 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4, 2.6, 2.8, 3.0, 1.0 };
    ...
    for (size_t i = 0; i < 11; ++i)
    {
        ...
        char szTitle[64];
        sprintf(szTitle, "Gamma=%.1f", pdblGammaCorrectonValues[i]);
...
int main(int /* argc */, char ** /* argv */)
{
    try
    {
        ...
        bool bLittleEndianCamera = true;
        bool enabledSwapEndian = false;
        ...
        if (pIEnumeration_DeviceRegistersEndianness.IsValid())
        {
            bLittleEndianCamera = (pIEnumeration_DeviceRegistersEndianness->GetCurrentEntry()->GetSymbolic().compare("Little") == 0);
            enabledSwapEndian = (!bLittleEndianHost && bLittleEndianCamera) || (bLittleEndianHost && !bLittleEndianCamera);
        }
		...
		*pIEnumeration_LineForegroundColorSelector = "Blue";
        ...
        thread worker(processGamma, &pIStDevice, &pIStGraphDisplayWnd, enabledSwapEndian, &isCompleted);
        while(!isCompleted)
        {
            // Check if display window is visible.
            ...
            if (!pIStGraphDisplayWnd->IsVisible())
            {
                pIStGraphDisplayWnd->Show(NULL, StWindowMode_Modaless);
            }
            processEventGUI();
        }
