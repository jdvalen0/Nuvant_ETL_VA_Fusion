...
//-----------------------------------------------------------------------------
void OnStCallbackCFunction(IStCallbackParamBase *pIStCallbackParamBase, UserParam_t pIStImageDisplayWnd)
{
...
void displayChoiceWorker(CIStDevicePtr *pIStDevice, CIStDataStreamPtr *pIStDataStream, bool *isCompleted)
{
    ...
    do
    {
        ...
        if (nInput <=2 && (nInput < sizeof(pisWritable) / sizeof(bool)) && pisWritable[nInput])
        {
...
int main(int, char **)
{
    try
    {
        ...
        // Initialize thread for displaying menu 
        bool isCompleted = false;
        thread worker(displayChoiceWorker, &pIStDevice, &pIStDataStream, &isCompleted);
